<?php
/**
 * Scraper PHP para jamichoacan.org.mx
...
// URL de jamichoacan.org.mx
$url = 'https://jamichoacan.org.mx/';
...
$events = parseJamichoacanContent($content);
...
function parseJamichoacanContent($content) {
    ...
    foreach ($lines as $line) {
        ...
        if (preg_match('/^- ([^:]+): ([^-]+)-\s*(\d{1,2}:\d{2}\s*(AM|PM))/i', $line, $matches)) {
            if ($currentEvent) {
                $events[] = $currentEvent;
            }
            
            $league = trim($matches[1]);
            $teams = trim($matches[2]);
            $time = trim($matches[3]);
            
            error_log("Found event: " . $league . " - " . $teams . " - " . $time);
            
            // Separar equipos
            $homeTeam = $teams;
            $awayTeam = '';
            if (preg_match('/(.+)\s*-\s*(.+)/', $teams, $teamMatch)) {
